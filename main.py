...
class Controller:
    ...
    pinchdirectionflag : bool
        true if pinch gesture movement is along x-axis, otherwise false
    prevpinchlv : int
        stores quantized magnitude of prev pinch gesture displacement, from starting position
    pinchlv : int
        stores quantized magnitude of pinch gesture displacement, from starting position
    framecount : int
        stores no. of frames since 'pinchlv' is updated.
    prev_hand : tuple
        stores (x, y) coordinates of hand in previous frame. pinch_threshold : float
    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                cv2.imshow('Virtual Mouse by SERB', image)
